...
- 構造化マークアップJSON-LD、`{記事ID}_schema.jsonld`、`Article`、`BreadcrumbList`、本文Q&A 5件と同期した必須の `FAQPage`、手順記事で追加する `HowTo`
- 一度に削除対象へできる記事は9件まで
...
- 完成報告前の標準実行監査。設定タブ、ニュース設定、KW記録、カテゴリ・タグ確認、Doc本文にタグ・出典セクションがないこと、構造化マークアップ、レビュー記録、画像・図解検証、内部リンク網、クラスター行、中央記事台帳、実行履歴が未達なら停止
